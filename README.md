...
| @Bot `/chatter [on\|off]` | 开关本群复读和热闹插话（仅管理员，默认关；不带参数则切换） |
| @Bot `/白名单 list\|add <群号>\|delete <群号>` | 管理群白名单（仅管理员，任意群可用，无需该群在白名单内） |
| @Bot `/help` | 显示帮助信息 |

白名单群里还有两类**不需要 @** 的主动行为（默认关闭，管理员 `@Bot /chatter on` 后生效）：

- **复读**：不同成员连续说同一句时，按句数指数概率跟读（2 句 10%，6 句 100%），同一轮只跟一次。
- **热闹插话**：按最近 5 分钟消息条数指数概率用当前人格接一句（1 条 1%，10 条 50%，20 条 100%）。Bot 在群里发过言后概率减半，5 分钟未再发言则重置。

"""
proactive 主动发言模块单元测试
──────────────────────────────
测试空闲计时器的重置/取消逻辑 + LLM 决策分支。
"""
...
class TestProactiveDecision:
    ...
    @pytest.mark.asyncio
    async def test_no_history_skips(self, tmp_session_dir):
        """对话历史为空时，不调用 LLM。"""
        _setup_handler_mock(tmp_session_dir, [])
        await proactive._try_proactive_message()
    ...
    @pytest.mark.asyncio
    async def test_llm_says_no(self, tmp_session_dir):
        """LLM 回复 NO → 不发消息。"""
        _setup_handler_mock(tmp_session_dir, [
    ...
    async def test_proactive_instruction_appended(self, tmp_session_dir):
        ...
        assert "现在是" in last_msg["content"] or "距离" in last_msg["content"]
        assert "NO" in last_msg["content"]
...
class TestTimerIntegration:
    ...
    async def test_timer_reset_extends_delay(self):
        ...
        with patch.object(proactive, "_try_proactive_message", new_callable=AsyncMock) as mock_try:
            ...
            proactive.reset_idle_timer()  # 在 0.1s 处重置
            await asyncio.sleep(0.15)     # 此时距重置仅 0.15s < 0.2s
            mock_try.assert_not_awaited()
            await asyncio.sleep(0.15)     # 总共 0.3s，距重置 0.2s
            mock_try.assert_awaited_once()
